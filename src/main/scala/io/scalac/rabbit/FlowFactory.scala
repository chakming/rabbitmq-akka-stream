...
trait FlowFactory extends LazyLogging {
  ...
  def consumerMapping: Flow[Delivery, ByteString, NotUsed] =
    Flow[Delivery].map(_.message.body.asInstanceOf[ByteString])
  
  /** Flow performing our domain processing. */
  def domainProcessing(implicit ex: ExecutionContext): Flow[ByteString, CensoredMessage, NotUsed] =
    Flow[ByteString]
      .map { _.utf8String }
  ...
  def publisherMapping: Flow[CensoredMessage, Routed, NotUsed] =
    Flow[CensoredMessage].map({
      case cMsg@MessageSafe(msg) => Routed(routingKey = outOkQueue.name, Message(body = ByteString(cMsg.message)))
      case cMsg@MessageThreat(msg) => Routed(routingKey = outNokQueue.name, Message(body = ByteString(cMsg.message)))
    })
